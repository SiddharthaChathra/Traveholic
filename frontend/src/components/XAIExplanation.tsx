...
import { 
  Brain, Zap, Target, Search, ChevronDown, ChevronUp, 
  AlertTriangle, TrendingUp, TrendingDown, Minus, Plus, 
  Circle, Check, X, Info, BarChart2, MapPin, Clock
} from 'lucide-react';
...
export function XAITab({ transects, selectedTransect }: XAITabProps) {
  ...
  const explanation = getMockExplanation(transect);
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeSection === 'behavior' && <BehaviorExplanation data={explanation.behavior} />}
        {activeSection === 'risk' && <RiskExplanation data={explanation.risk} />}
        {activeSection === 'forecast' && <ForecastExplanation data={explanation.forecast} />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 p-4 bg-white rounded-lg border border-slate-200">
        <div className={cn('px-4 py-2 rounded-full text-lg font-bold', classColors[data.class_id] || classColors[0])}>
...
function RiskExplanation({ data }: { data: any }) {
  ...
  return (
    <div className="space-y-6">
      {/* Score Card */}
      <div className={`p-6 rounded-xl border border-slate-200 ${scoreBg}`}>
...
function getMockExplanation(transect: any) {
  ...
  return {
    behavior: {
      ...
      natural_language: 'This transect shows clear erosive behavior with a retreat rate of 0.42 m/year. The trend is statistically significant (p<0.01). Seasonal oscillation is moderate but does not offset the long-term loss. This area may need intervention.',
    },
    risk: {
    ...
    forecast: {
      forecast: {
        7: { position: 94.8, lower: 94.2, upper: 95.4, confidence: 0.92 },
        30: { position: 94.1, lower: 93.0, upper: 95.2, confidence: 0.85 },
        90: { position: 92.8, lower: 90.5, upper: 95.1, confidence: 0.72 },
        180: { position: 91.2, lower: 87.5, upper: 94.9, confidence: 0.60 },
        365: { position: 89.5, lower: 82.0, upper: 97.0, confidence: 0.45 },
      },
      monsoon_context: 'Monsoon season active - expect accelerated erosion June-September',
