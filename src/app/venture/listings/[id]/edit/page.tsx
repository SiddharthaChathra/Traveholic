...
export default function EditListingPage({ params }: { params: { id: string } }) {
  ...
  // pre-populate values simulating listing fetch
  const [name, setName] = useState('Grand Oceanfront Deluxe Suite');
  const [description, setDescription] = useState('Experience unparalleled beachfront views and direct garden walkway access.');
  const [category, setCategory] = useState('hotel');
  const [location, setLocation] = useState('Nusa Dua, Bali');

  const [images, setImages] = useState<string[]>([
    'https://images.unsplash.com/photo-1542314831-068cd1dbfeeb?w=600&auto=format&fit=crop&q=80'
  ]);
  const [selectedAmenities, setSelectedAmenities] = useState<string[]>(['Free WiFi', 'Swimming Pool', 'Ocean View']);
  ...
  const [priceMin, setPriceMin] = useState(220);
  ...
  const handleUploadImages = (files: File[]) => {
    const mockUrls = files.map((file) => `https://images.unsplash.com/photo-1566073771259-6a8506099945?w=600&auto=format&fit=crop&q=80`);
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '32px' }}>
      <div>
        <h2 style={{ fontSize: '22px', fontWeight: 800, margin: 0, color: 'var(--text-primary)' }}>Edit Listing</h2>
        <p style={{ color: 'var(--text-secondary)', fontSize: '13px', marginTop: '4px' }}>Modify listing information, pricing slots, and policies.</p>
      </div>
      ...
      <div className="discover-premium-card" style={{ padding: '28px', borderRadius: '16px', background: 'var(--card-bg)', border: '1px solid var(--card-border)' }}>
        ...
        {step === 5 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, margin: 0, color: 'var(--text-primary)' }}>Review &amp; Save</h3>
            ...
            <div style={{ padding: '16px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.04)', display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                ...
                <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', display: 'block' }}>{name}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span style={{ fontSize: '13px', color: 'var(--text-primary)', display: 'block' }}>{location}</span>
              </div>
              <div>
        ...
        <div style={{ marginTop: '32px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '20px' }}>
          <ThreeTierButtonGroup 
            buttons={[
              ...
              {
                label: step === 5 ? 'Save Changes' : 'Next Step',
                onClick: () => {
                  if (step === 5) {
