...
export default function Home() {
  ...
  const { user, savedAccounts, switchAccount, loading, login, signup, logout, refreshSavedAccounts, updateTravellerType, forgotPassword, verifyOtp, resetPassword } = useAuth();
  const router = useRouter();

  const searchParams = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null;
  const [showAddAccount, setShowAddAccount] = useState(searchParams?.get('action') === 'add_account');
  useEffect(() => {
  ...
  useEffect(() => {
    if (!loading && user && !showAddAccount) {
      if (user.role === 'business') {
        router.push('/venture/dashboard');
      }
    }
  }, [user, loading, router, showAddAccount]);
  ...
  const [streamStatus, setStreamStatus] = useState<'idle' | 'connecting' | 'live' | 'nodata'>('idle');
  ...
  const [liveChatMessages, setLiveChatMessages] = useState([
    ...
    { id: '2', username: 'flora_explores', text: '¿Puedes mostrar el equipo de cámara que estás usando?', translatedText: 'Can you show the camera gear you are using?', timestamp: '20:46' },
    { id: '3', username: 'nomad_vlogs', text: 'C’est tellement fluide ! Stream de haute qualité.', translatedText: 'This looks so smooth! High quality stream.', timestamp: '20:47' },
  ...
  useEffect(() => {
    ...
    if (liveSource === 'webcam' && streamStatus === 'live') {
  ...
  useEffect(() => {
    if (liveMediaStream) {
      ...
      if (pipVideoRef.current) {
        pipVideoRef.current.srcObject = liveMediaStream;
        pipVideoRef.current.play().catch(err => console.log("PiP Video playback interrupted", err));
      }
    }
  }, [liveMediaStream, liveVideoRef.current, pipVideoRef.current]);
  ...
  const toggleLikeVlog = (id: string) => {
    setVlogs((prev) => {
      let newLikes = 0;
      const nextVlogs = prev.map((v) => {
        if (v.id === id) {
          newLikes = v.liked ? v.likes - 1 : v.likes + 1;
          return {
            ...v,
            liked: !v.liked,
            likes: newLikes
      ...
      if (newLikes > 0 || newLikes === 0) { // update likeCounts if we found it
        setLikeCounts((prevCounts) => ({ ...prevCounts, [id]: newLikes }));
      }
      return nextVlogs;
    });
  };
    ...
    fetch('/api/stories', { headers })
      ...
      .then(data => {
        if (Array.isArray(data)) {
          // Backend returns grouped stories: [{ user: {username, avatarUrl}, stories: [...] }]
          // Frontend expects a flat array of story items for the viewer.
          // For the feed top bar, it probably maps over them. Wait, if it maps over them, it might expect one per user or just flatten them.
          // The current frontend has 1 story per user in the mock data. Let's flatten them.
          const flatStories = data.flatMap(group =>
            group.stories.map((s: any) => ({
              ...
              image: s.imageUrl,
              caption: s.caption || '',
              viewed: !group.hasUnread, // Simplify for now
              isLive: false
            }))
          );
      ...
      .catch(err => console.error("Error loading stories from DB:", err));
  }, []);
      ...
      .then(data => {
        if (Array.isArray(data.data)) {
          const mapped = data.data.map((n: any) => ({
            ...
            type: n.type.toLowerCase(), // 'like', 'comment', 'follow'
            text: n.type === 'LIKE' ? 'liked your post.' : n.type === 'COMMENT' ? 'commented on your post.' : 'started following you.',
            time: new Date(n.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
          }));
          setNotifications(mapped);

          // Assuming API returns unread count or we can count them, for now just use the array length or simple logic
          setUnreadNotifications(data.data.filter((n: any) => !n.isRead).length);
        }
      })
      .catch(err => console.error("Error loading notifications from DB:", err));
  }, []);
  ...
  const fetchAndMapFeed = (cursor: string | null = null) => {
    const token = localStorage.getItem('travora_token');
    const headers: Record<string, string> = {};
    if (token) headers['Authorization'] = `Bearer ${token}`;
    ...
    return fetch(url, { headers })
      .then(res => res.json())
      .then(result => {
        const data = result.feed || result.data || result;
        if (Array.isArray(data)) {
          const mapped = data.map(post => {
            let imagesArr: string[] = [];
            try {
              if (post.images && post.images.startsWith('[')) {
            ...
            let username = post.user?.username || post.username;
            let avatar = post.user?.avatar || post.avatar;
            if (!username) {
              if (post.id === 'post-1') {
                username = 'wanderlust_jenny';
                avatar = 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=100&auto=format&fit=crop&q=80';
            ...
            return {
              ...
              avatar,
              hashtags,
              likesCount: post.likes !== undefined ? post.likes : (post.likesCount || 0),
              images: imagesArr,
          ...
          return { mapped, nextCursor: result.nextCursor || null };
        }
        return { mapped: [], nextCursor: null };
      });
  };

  // Fetch social feed posts on initial load
  useEffect(() => {
    fetchAndMapFeed().then(({ mapped, nextCursor }) => {
      setPosts(mapped);
      setFeedCursor(nextCursor);
    }).catch(err => console.error("Error loading feed from DB:", err));
  }, []);
  ...
  const toggleLikeReel = (reelId: string) => {
    ...
    if (isCurrentlyLiked) {
      ...
      setReels(curr => curr.map(r => r.id === reelId ? { ...r, likes: Math.max(0, r.likes - 1) } : r));
    } else {
      setLikedReels(prev => {
        const next = new Set(prev);
        next.add(reelId);
  ...
  // Fetch dynamic user suggestions
  useEffect(() => {
    const token = localStorage.getItem('travora_token');
    const headers: Record<string, string> = {};
    if (token) headers['Authorization'] = `Bearer ${token}`;
    ...
    fetch('/api/user/suggestions', { headers })
      ...
      .then(data => {
        if (Array.isArray(data.suggestions)) {
          const mapped = data.suggestions.map((u: any) => ({
            id: u.id,
            name: u.fullName || u.username,
            username: u.username,
            bio: u.bio || 'New to Travora! 👋',
            avatar: u.avatarUrl || '👤',
            mutualAvatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=80', // placeholder mutual
            mutualText: u.mutualFollowers > 0 ? `${u.mutualFollowers} mutual followers` : 'Suggested for you',
            isOpenToWork: false,
      ...
      .catch(err => console.error("Error loading suggested people:", err));
  }, []);
  ...
  const toggleLike = async (id: string) => {
    ...
    if (isCurrentlyLiked) {
      ...
      setPosts((currentPosts) =>
        currentPosts.map((p) => (p.id === id ? { ...p, likesCount: Math.max(0, p.likesCount - 1) } : p))
      );
    } else {
      setLikedPosts((prev) => {
        const next = new Set(prev);
  ...
  const handleMediaUpload = async (files: File[]) => {
    ...
    try {
      ...
      if (res.ok && data.success && data.files) {
        data.files.forEach((uploadedFile: any) => {
          const newItem = {
          ...
          setPostImages(prev => [...prev, uploadedFile.url]);
        });
  ...
  const openUserProfile = (username: string) => {
    ...
    if (username === (user ? `@${user.username}` : '@travel_buddy')) {
    ...
    try {
      if (formMode === 'forgot_password') {
        ...
        if (result.success) {
          ...
          if (showAddAccount) {
            // Close the add-account modal overlay
            setShowAddAccount(false);
            window.history.replaceState({}, '', '/');
            // Refresh saved accounts so the newly added one appears in the dropdown
            await refreshSavedAccounts();
          }
          setSuccessMsg(showAddAccount ? 'Account added! You can now switch to it from Switch Accounts.' : 'Logged in successfully!');
          setIdentifier('');
          setPassword('');
        } else {
        ...
        if (result.success) {
          ...
          setSuccessMsg(showAddAccount ? 'New account created and added! Switch to it from Switch Accounts.' : 'Account created successfully!');
          setUsername('');
          setEmail('');
          setPassword('');
          setFullName('');
  ...
  if (user && !showAddAccount) {
    ...
    console.log("RENDER CHECK -> streamStatus:", streamStatus, "activeTab:", activeTab, "Should render PiP:", (streamStatus === 'connecting' || streamStatus === 'live') && activeTab !== 'live-dashboard');
    ...
    return (
      <>
                    ...
                    {isLoadingFeed ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                        ...
                        <div className="feed-infinite-scroll-trigger">
                          ...
                          ) : (visiblePostsCount < posts.length || feedCursor) ? (
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (visiblePostsCount < posts.length) {
                                  ...
                                  fetchAndMapFeed(feedCursor).then(({ mapped, nextCursor }) => {
                                    setPosts(prev => [...prev, ...mapped]);
                                    setFeedCursor(nextCursor);
                                    setIsLoadingMore(false);
                                    setVisiblePostsCount(prev => prev + mapped.length);
                                  }).catch(() => {
                                    setIsLoadingMore(false);
                                  });
                                }
                                  ...
                                  <div className="switch-accounts-list">
                                    {savedAccounts?.map((account, idx) => (
                                      <motion.div
                                        ...
                                        className={`switch-account-item ${user?.id === account.userId ? 'active' : ''}`}
                                        onClick={async () => { 
                                          if (user?.id !== account.userId) {
                                            if (account.sessionExpired) {
                                              showToast(`Session expired for @${account.username}. Please log in again.`);
                                            } else {
                                      ...
                                      >
                                        <motion.div
                                          ...
                                          style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'var(--brand-gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '16px', flexShrink: 0 }}
                                        >
                                          {(account.username)[0].toUpperCase()}
                                        </motion.div>
                                        <motion.div variants={textVariants} style={{ flex: 1, minWidth: 0, marginLeft: '12px' }}>
                                          <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>@{account.username}</div>
                                          <div style={{ fontSize: '10px', color: account.sessionExpired ? '#ef4444' : 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                            {account.sessionExpired ? 'Session expired — tap to re-login' : `${account.fullName} ${user?.id === account.userId ? '(Active)' : ''}`}
                                          </div>
                                        </motion.div>
                                        {user?.id === account.userId && (
                                          <motion.div variants={checkmarkVariants} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginLeft: '8px' }}>
                                    ...
                                    <motion.div
                                      variants={itemVariants}
                                      className="switch-account-item add-account-btn"
                                      onClick={() => {
                                        setShowSwitchDropdown(false);
                                    ...
                                    >
                                      <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-primary)', flexShrink: 0 }}>
                                      ...
                                      <div style={{ flex: 1, minWidth: 0, marginLeft: '12px' }}>
                                        <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-primary)' }}>Log into Existing Account</div>
                                      </div>
                                    </motion.div>
                                  </div>
                                </motion.div>
                            ...
                            <div className="phone-post-card">
                              ...
                              <div className="phone-post-header">
                                <div className="user-profile-info">
                                  ...
                                  <div className="user-text-meta">
                                    <span className="preview-username">@{user?.username || 'travel_buddy'}</span>
                              ...
                              <div className="phone-caption-section">
                                <div className="caption-text-block">
                                  <span className="caption-username">@{user?.username || 'travel_buddy'}</span>{' '}
                    ...
                    <div className="discover-premium-card" style={{ padding: '16px', borderRadius: '16px', border: '1px solid var(--card-border)', background: 'var(--card-bg)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                      ...
                      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 0', display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '380px' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '4px' }}>
                          <button
                            type="button"
                            onClick={() => setIsChatTranslated(!isChatTranslated)}
                            style={{ background: isChatTranslated ? 'var(--primary)' : 'rgba(255,255,255,0.1)', border: 'none', padding: '4px 8px', borderRadius: '4px', color: '#fff', fontSize: '10px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', transition: 'background 0.2s' }}
                          >
                            ...
                            {isChatTranslated ? 'Translation On' : 'Auto-Translate'}
                          </button>
                        </div>
                        {liveChatMessages.map((msg) => (
                          <div key={msg.id} style={{ fontSize: '12px', lineHeight: '1.4' }}>
                            <span style={{ fontWeight: 700, color: msg.username === (user?.username || 'travel_buddy') ? 'var(--primary)' : 'var(--text-secondary)' }}>@{msg.username}</span>{' '}
                            <span style={{ color: 'var(--text-primary)' }}>
                              {isChatTranslated && msg.translatedText ? msg.translatedText : msg.text}
                              {isChatTranslated && msg.translatedText && (
                                <span style={{ fontSize: '10px', color: 'var(--text-muted)', marginLeft: '4px' }}>(translated)</span>
                              )}
                            </span>
                          </div>
                        ))}
                      ...
                      <form
                        onSubmit={(e) => {
                          ...
                          const newMsg = {
                            ...
                            username: user?.username || 'travel_buddy',
                            text: liveChatInput,
                            timestamp: '20:47'
                          };
        ...
        {/* Floating PiP Live Stream Player (Moved outside app-container to prevent clipping) */}
        {(streamStatus === 'connecting' || streamStatus === 'live') && activeTab !== 'live-dashboard' && (
          <div 
            style={{
              position: 'fixed',
              bottom: '150px',
          ...
          >
            <div style={{ position: 'relative', height: '150px', background: 'var(--input-bg)' }}>
              {liveSource === 'webcam' && streamStatus === 'live' ? (
                <video ref={pipVideoRef} autoPlay playsInline muted style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : liveThumbnail ? (
                 <img src={liveThumbnail} alt="Live Stream" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                 <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
                    No Camera
                 </div>
              )}
              <div style={{ position: 'absolute', top: '8px', left: '8px', background: 'var(--brand-gradient)', color: 'white', padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: 800, textTransform: 'uppercase' }}>
                {streamStatus === 'connecting' ? 'CONNECTING...' : 'LIVE'}
              </div>
            </div>
            <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ fontSize: '14px', fontWeight: 700, color: 'var(--text-primary)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                {liveTitle || 'Untitled Stream'}
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <button 
                  onClick={() => setActiveTab('live-dashboard')} 
                  style={{ flex: 1, padding: '8px', background: 'var(--brand-gradient)', color: 'white', border: 'none', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}
                >
                  Return
                </button>
                <button 
                  ...
                  style={{ padding: '8px 12px', background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '6px', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}
                >
                  End
                </button>
              </div>
